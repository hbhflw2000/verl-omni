...
def test_ar_rollout_hard_fails_raw_logprobs_when_processed_required(monkeypatch):
    monkeypatch.setenv("VERL_OMNI_REQUIRE_PROCESSED_ROLLOUT_LOGPROBS", "1")
    monkeypatch.delenv("VERL_OMNI_REQUIRE_RAW_ROLLOUT_LOGPROBS", raising=False)
...
def test_ar_rollout_hard_fails_processed_logprobs_when_raw_required(monkeypatch):
    monkeypatch.setenv("VERL_OMNI_REQUIRE_RAW_ROLLOUT_LOGPROBS", "1")
    monkeypatch.delenv("VERL_OMNI_REQUIRE_PROCESSED_ROLLOUT_LOGPROBS", raising=False)
...
def test_ar_rollout_hard_fails_conflicting_logprob_requirements(monkeypatch):
    monkeypatch.setenv("VERL_OMNI_REQUIRE_RAW_ROLLOUT_LOGPROBS", "1")
    monkeypatch.setenv("VERL_OMNI_REQUIRE_PROCESSED_ROLLOUT_LOGPROBS", "1")
    ...
    with pytest.raises(ValueError, match="cannot require both"):
        server._validate_configs()
...
def test_preprocess_engine_kwargs_propagates_ar_startup_timeouts(monkeypatch):
    monkeypatch.delenv("VERL_OMNI_VLLM_STARTUP_HANDSHAKE_TIMEOUT", raising=False)
    ...
    assert os.environ["VERL_OMNI_VLLM_STARTUP_HANDSHAKE_TIMEOUT"] == "1800"
...
def test_run_server_preserves_ar_startup_timeouts_after_engine_arg_parse(monkeypatch):
    ...
    server._configure_vllm_internal_port_base = lambda: None
    server._configure_omni_distributed_args = lambda args, headless=False: None
    server._reserve_vllm_worker_master_port = lambda: (29500, _FakeSocket())
...
def test_vllm_port_actor_slot_claims_distinct_same_host_slots(monkeypatch, tmp_path):
    monkeypatch.setenv("VERL_OMNI_VLLM_PORT_LOCK_DIR", str(tmp_path))

    server_a = object.__new__(vLLMOmniHttpServer)
    server_b = object.__new__(vLLMOmniHttpServer)

    try:
        slot_a = server_a._claim_vllm_port_actor_slot(seed="unit_job", block_index=0, actor_slots=4)
        slot_b = server_b._claim_vllm_port_actor_slot(seed="unit_job", block_index=0, actor_slots=4)

        assert slot_a == 0
        assert slot_b == 1
        assert server_a._claim_vllm_port_actor_slot(seed="unit_job", block_index=0, actor_slots=4) == 0
    finally:
        for server in (server_a, server_b):
            fd = getattr(server, "_vllm_port_slot_lock_fd", None)
            if fd is not None:
                os.close(fd)


def test_configure_omni_distributed_args_uses_controlled_ports(monkeypatch):
    monkeypatch.setenv("VERL_OMNI_MASTER_ZMQ_PORT_BASE", "60392")
    monkeypatch.setenv("VERL_OMNI_VLLM_DIST_MASTER_ADDR", "10.66.164.18")
    monkeypatch.setenv("VERL_OMNI_VLLM_DIST_MASTER_PORT", "65322")
...
def test_configure_omni_distributed_args_preserves_legacy_port_without_env(monkeypatch):
    ...
    monkeypatch.delenv("VERL_OMNI_MASTER_ZMQ_PORT_BASE", raising=False)
    monkeypatch.delenv("VERL_OMNI_VLLM_DIST_MASTER_PORT", raising=False)
...
def test_ar_preprocess_input_normalizes_true_logprobs_to_vllm_default():
    server = object.__new__(vLLMOmniHttpServer)
    server._ar_mode = True
    server.config = _Config(
        max_model_len=16,
