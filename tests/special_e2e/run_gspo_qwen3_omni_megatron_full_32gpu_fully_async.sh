...
MODEL_EXTERNAL_LIB=${MODEL_EXTERNAL_LIB:-verl_omni.models.transformers.qwen3_omni_thinker}
export VERL_USE_EXTERNAL_MODULES=${VERL_USE_EXTERNAL_MODULES:-verl_omni,${MODEL_EXTERNAL_LIB}}
...
export PYTHONPATH="${REPO_ROOT}:${VERL_ROOT}:${MEGATRON_BRIDGE_REPO}/3rdparty/Megatron-LM:${MEGATRON_BRIDGE_REPO}/src:${VLLM_OMNI_ROOT}:${PYTHONPATH:-}"
...
STAGE_CONFIG=${STAGE_CONFIG:-${SCRIPT_DIR}/qwen3_omni_thinker_only_tp4_full_async_no_sleep.yaml}
...
claim_ray_worker_port_block() {
  ...
  fail "No free local Ray worker port block in ${pool_start}-${pool_end} with span=${span}"
}
...
validate_static_config() {
  ...
  for name in NNODES GPUS_PER_NODE TRAIN_NNODES TRAIN_GPUS_PER_NODE ROLLOUT_NNODES \
    ...
    ROLLOUT_MAX_NUM_BATCHED_TOKENS ASYNC_TRIGGER_PARAMETER_SYNC_STEP \
    STAGE_INIT_TIMEOUT INIT_TIMEOUT \
    VERL_OMNI_VLLM_STARTUP_HANDSHAKE_TIMEOUT RAY_NODE_CPUS \
    RAY_START_WAIT_SECONDS RAY_WORKER_PORT_SPAN; do
    require_positive_int "${name}"
  done
...
ACTOR_CLIP_RATIO_C=${ACTOR_CLIP_RATIO_C:-3.0}
ACTOR_LOSS_AGG_MODE=${ACTOR_LOSS_AGG_MODE:-token-mean}
ACTOR_OPTIM_CLIP_GRAD=${ACTOR_OPTIM_CLIP_GRAD:-1.0}
...
ASYNC_STALENESS_THRESHOLD=${ASYNC_STALENESS_THRESHOLD:-0}
...
export VERL_OMNI_REQUIRE_RAW_ROLLOUT_LOGPROBS=${VERL_OMNI_REQUIRE_RAW_ROLLOUT_LOGPROBS:-0}
...
start_ray_cluster() {
  ...
  if [[ "${NODE_RANK}" == "0" ]]; then
    echo "[info] Starting Ray head at ${RAY_NODE_IP_ADDRESS}:${RAY_PORT}"
    ray_head_args=(
      --head
    ...
    until ray start \
      ...
      echo "[warn] Ray worker join failed; retry in 10s"
      sleep 10
    done
...
print(f"[info] Connecting to Ray at {address}", flush=True)
...
while True:
    ...
    print(
        ...
        f"node_keys={node_keys}, resources={resources}, available={available}",
        flush=True,
    )
    print(
        "[info] Ray nodes: "
        + "; ".join(
            f"{node.get('NodeManagerAddress')} alive={node.get('Alive')} "
    ...
    if time.time() - start > timeout:
        raise SystemExit(f"Timed out waiting for Ray resources: GPU={gpus}/{expected_gpus}")
    time.sleep(10)
ray.shutdown()
PY
}
...
echo "[info] TOTAL_TRAINING_STEPS=${TOTAL_TRAINING_STEPS} TOTAL_ROLLOUT_STEPS=${TOTAL_ROLLOUT_STEPS} REQUIRE_BATCHES=${REQUIRE_BATCHES} ASYNC_TRIGGER_PARAMETER_SYNC_STEP=${ASYNC_TRIGGER_PARAMETER_SYNC_STEP}"
...
echo "[info] actor lr=${ACTOR_LR} use_kl_loss=${ACTOR_USE_KL_LOSS} kl_loss_coef=${ACTOR_KL_LOSS_COEF} clip=${ACTOR_CLIP_RATIO_LOW}/${ACTOR_CLIP_RATIO_HIGH}/${ACTOR_CLIP_RATIO_C} loss_agg=${ACTOR_LOSS_AGG_MODE} grad_clip=${ACTOR_OPTIM_CLIP_GRAD}"
echo "[info] rollout tp=${ROLLOUT_TP} dp=${ROLLOUT_DP} replicas=$((ROLLOUT_WORLD_GPUS / (ROLLOUT_TP * ROLLOUT_DP))) n=${N_RESP_PER_PROMPT} corr_bypass=${ROLLOUT_CORR_BYPASS_MODE} staleness=${ASYNC_STALENESS_THRESHOLD} partial=${ASYNC_PARTIAL_ROLLOUT}"
...
echo "[info] sequence_parallel=${SEQUENCE_PARALLEL} attention_backend=${ATTENTION_BACKEND}"
...
echo "[info] Ray ports: head=${RAY_PORT} head_pool=${RAY_HEAD_PORT_POOL_START}-${RAY_HEAD_PORT_POOL_END} head_negotiation=${RAY_HEAD_PORT_NEGOTIATE} head_file=${RAY_HEAD_PORT_FILE} seed=${RAY_PORT_SEED} dashboard_enabled=${RAY_INCLUDE_DASHBOARD} dashboard=${RAY_DASHBOARD_PORT} worker_range=${RAY_MIN_WORKER_PORT}-${RAY_MAX_WORKER_PORT} worker_pool=${RAY_WORKER_PORT_POOL_START}-${RAY_WORKER_PORT_POOL_END} worker_seed=${RAY_WORKER_PORT_SEED} worker_span=${RAY_WORKER_PORT_SPAN} worker_preferred_block=${RAY_WORKER_PORT_PREFERRED_BLOCK_INDEX} worker_block=${RAY_WORKER_PORT_BLOCK_INDEX} worker_node_rank_hint=${RAY_WORKER_PORT_NODE_RANK_HINT} worker_local_probe=${RAY_WORKER_PORT_LOCAL_PROBE}"
...
python3 -m verl.experimental.fully_async_policy.fully_async_main \
    ...
    actor_rollout_ref.actor.optim.clip_grad="${ACTOR_OPTIM_CLIP_GRAD}" \
    ...
    ++ray_kwargs.ray_init.runtime_env.env_vars.VERL_FULLY_ASYNC_TRAINER_NUM_CPUS=\"${VERL_FULLY_ASYNC_TRAINER_NUM_CPUS:-10}\" \
    ++ray_kwargs.ray_init.runtime_env.env_vars.VERL_FULLY_ASYNC_ROLLOUTER_NUM_CPUS=\"${VERL_FULLY_ASYNC_ROLLOUTER_NUM_CPUS:-10}\" \
